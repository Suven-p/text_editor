...
enum class LangType { L_C, L_CPP, L_USER, L_TXT };
...
public:
	...
	Buffer(const Buffer& buf) :m_is_dirty(buf.m_is_dirty), m_doc(buf.m_doc), m_language(buf.m_language),
		m_read_only(buf.m_read_only), m_pos(buf.m_pos)
	{
