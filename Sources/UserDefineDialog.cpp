...
void UserDefineDialog::changeStyle()
{
	...
	::SetWindowPos(m_hwnd, HWND_TOP, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOOWNERZORDER | SWP_FRAMECHANGED | SWP_NOREPOSITION);
...
BOOL CALLBACK UserDefineDialog::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		else
		{
			...
			//	bool isChecked = (BST_CHECKED == ::SendMessage(::GetDlgItem(m_hwnd, int(wParam)), BM_GETCHECK, 0, 0));
